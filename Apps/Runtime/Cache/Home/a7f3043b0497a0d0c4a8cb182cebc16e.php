 ...
 <script>
   ...
   $(document).ready(function(){
	...
	<style type="text/css">
		...
		.catelist li a{display: block;padding-left: 30px; font-size: 14px;}
		...
		.main_r .articlelist li p{margin: 0;text-indent: 2em;line-height: 18px;color: #8c8c8c;font-size: 12px;}
...
<div style="border:1px dashed #dfdfdf;border-bottom: none;font-size: 16px;color:#333;padding: 5px 10px;font-weight: bold;"><?php echo L('site_category');?></div>
<div style="width: 228px;padding:10px;height: auto;border: 1px dashed #dfdfdf;">
	<ul class="catelist">
		<?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><li><a href="<?php echo U('Cate/lists','id='.$c['id']);?>" <?php if(($c["id"]) == $articleinfo["cid"]): ?>style="background:#f2f2f2;"<?php endif; ?>><?php echo ($c["title"]); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?>
	</ul>
</div>
</div>
...
<div style="border:1px dashed #dfdfdf;border-bottom: none;font-size: 14px;color:#333;padding: 5px 10px;"><?php echo L('your_position');?>：<a href="<?php echo U('Index/index');?>"><?php echo L('site_index');?></a> > <a href="<?php echo U('Cate/lists','id='.$articleinfo['cid']);?>"><?php echo ($articleinfo["catetitle"]); ?></a> > <?php echo ($articleinfo["title"]); ?></div>
</div>
<style type="text/css">
		p{display: block;margin-bottom: 10px;}
	</style>
<div style="width: 898px;margin-left:270px;border: 1px dashed #ccc;padding:15px;min-height: 400px;">
	<div style="font-size: 16px;font-weight: bold;text-align: center;border-bottom: 1px solid #ccc;padding-bottom: 15px;"><?php echo ($articleinfo["title"]); ?></div>
