			...
			<table id="sample-table-1" class="table table-striped table-bordered table-hover">
				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td>
								<?php echo ($v["user"]["username"]); ?> (<?php echo ($v["user"]["nickname"]); ?>)
								<br>
								<span class="action-buttons">							
								<a class="blue replymsg" href="###" uid="<?php echo ($v["id"]); ?>"> 回复
								</a>
								<a class="red delliuyan" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;" title="删除"> <i class="icon-trash bigger-130"></i>
								</a>
							</span>
							</td>
							<td><?php echo ($v["content"]); ?> [<?php echo (date("Y-m-d",$v["add_time"])); ?>] <br>
								<span class="replycontent" style="display: inline-block;color:red;"><?php if($v["reply"] != ''): echo ($v["mem"]["nickname"]); ?> 回复：<?php echo ($v["reply"]); ?> [<?php echo (date("Y-m-d",$v["reply_time"])); ?>]<?php endif; ?></span>
								<div class="reply" style="display: none;">
									<textarea name="reply" class="replyarea" style="width:100%;margin-bottom: 5px;" rid="<?php echo ($v["id"]); ?>"><?php echo ($v["reply"]); ?></textarea>
...
$(function(){
		$('.delliuyan').click(function(){
					...
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('del');?>",{'id':id,'ajax':1,'type':'liuyan'},function(data){
								if (data.status) {
									$.gritter.add({
	...
	$('.tjreply').click(function(){
		...
		$.post('<?php echo U("reply");?>',{'reply':reply,'id':rid},succes,'json');
