...
<html>
	...
	<div class="row">
		<div class="col-sm-6">
			...
			<form <?php if($info): ?>action="<?php echo U('payedit');?>"
				<?php else: ?>
				action="<?php echo U('payadd');?>"<?php endif; ?>
	...
	<div class="col-xs-12 col-sm-9">
		<div class="clearfix">
			<?php if($info): ?><input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
				...
				<button type="submit" class="btn btn-success btn-next no-border">确认编辑</button><a href="<?php echo U('pay');?>" class="btn btn-light btn-next no-border">取消</a>
				<?php else: ?>
...
<tbody>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			...
			<td class="action-buttons">
				...
				<a class="blue" href="<?php echo U('pay','id='.$v['id']);?>">
					<i class="icon-pencil bigger-130"></i>
				</a>

				<a class="red paydel" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;">
...
jQuery(function($){
	$('.paydel').click(function(){
					...
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('paydel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
