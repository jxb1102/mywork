...
<html>
	...
	<div class="row">
<div class="col-xs-12">
<a href="<?php echo U('Post/goodlistadd');?>" class="btn btn-success btn-sm btn-next no-border">添加商城</a>
...
<table id="sample-table-1" class="table table-striped table-bordered table-hover">
	...
	<tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><a href="<?php echo U('Post/goodlist','fid='.$v['fid']);?>"><?php echo ($v["cate"]["title"]); ?></a></td>
				<td><?php echo ($v["start"]); ?>&nbsp;星</td>
				<td><?php echo (getstatus($v["state"])); ?></td>
				<td><?php echo ($v["sort"]); ?></td>
				<td class="action-buttons">
					<a class="blue" href="<?php echo U('Post/goodlistadd','id='.$v['id']);?>"> <i class="icon-pencil bigger-130"></i>
					</a>
					<a class="red goodlistdel" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;"> <i class="icon-trash bigger-130"></i>
					</a>
...
jQuery(function($){
	$('.goodlistdel').click(function(){
					...
					bootbox.confirm("确定要删除该文章么?", function(result){
						if(result) {
							$.post("<?php echo U('goodlistdel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
