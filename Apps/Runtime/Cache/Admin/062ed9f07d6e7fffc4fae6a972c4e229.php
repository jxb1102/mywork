...
<html>
	...
	<div class="row">
<div class="col-xs-12">
<a href="<?php echo U('Post/addarticle');?>" class="btn btn-success btn-sm btn-next no-border">添加文章</a>
...
<table id="sample-table-1" class="table table-striped table-bordered table-hover">
	...
	<tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
				...
				<td><a href="<?php echo U('Post/article','cid='.$v['cid']);?>"><?php echo ($v["cate"]["title"]); ?></a></td>
				<td class="hidden-480"><?php echo ($v["mem"]["nickname"]); ?></td>
				<td><?php echo (getstatus($v["status"])); ?></td>
				<td><?php echo ($v["sort"]); ?></td>
				<td class="hidden-480"><?php echo (date("Y-m-d",$v["addtime"])); ?></td>
				<td class="hidden-480"><?php echo (date("Y-m-d",$v["edittime"])); ?></td>
				<td class="action-buttons">
					<a class="blue" href="<?php echo U('Post/editarticle','id='.$v['id']);?>"> <i class="icon-pencil bigger-130"></i>
					</a>
					<a class="red delarticle" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;"> <i class="icon-trash bigger-130"></i>
					</a>
...
jQuery(function($){
	$('.delarticle').click(function(){
					...
					bootbox.confirm("确定要删除该文章么?", function(result){
						if(result) {
							$.post("<?php echo U('articledel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
