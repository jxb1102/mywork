				...
				<tbody>
					<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
							...
							<td class="action-buttons">
								<a class="red senddel" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;" title="删除"> <i class="icon-trash bigger-130"></i>
								</a>
							</td>
...
<form class="form-horizontal" id="noticeadd" style="padding:10px;" method="post" action="<?php echo U('sendadd');?>">
...
$(function(){
		$('.senddel').click(function(){
					...
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('senddel');?>",{'id':id,'ajax':1,'type':'send'},function(data){
								if (data.status) {
									$.gritter.add({
