...
<html>
	...
	<div class="row">
		<div class="col-sm-6">
			...
			<form <?php if($info): ?>action="<?php echo U('groupedit');?>"
				<?php else: ?>
				action="<?php echo U('groupadd');?>"<?php endif; ?>
	...
	<div class="form-group">
		...
		<div class="col-xs-12 col-sm-9">
			<div class="clearfix">
				<?php if($info): ?><button type="submit" class="btn btn-success btn-next no-border">确认编辑</button><a href="<?php echo U('group');?>" class="btn btn-light btn-next no-border">取消</a>
	...
	<tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
				...
				<td class="action-buttons">
					<a href="<?php echo U('group','access='.$v['id']);?>">分配权限</a>
					<a class="blue" href="<?php echo U('group','id='.$v['id']);?>">
						<i class="icon-pencil bigger-130"></i>
					</a>
                    <?php if($v["id"] != 1): ?><a class="red delgroup" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;">
...
jQuery(function($){
	$('.delgroup').click(function(){
					...
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('groupdel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
