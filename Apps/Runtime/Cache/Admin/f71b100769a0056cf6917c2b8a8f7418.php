...
<html>
	...
	<div class="row">
<div class="col-xs-12">
<a href="<?php echo U('Msg/activeadd');?>" class="btn btn-success btn-sm btn-next no-border">添加活动商品类</a>
	...
	<tbody>
		<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
				...
				<td class="action-buttons">
					<a class="blue" href="<?php echo U('Msg/activeadd','id='.$v['id']);?>"> <i class="icon-pencil bigger-130"></i>
					</a>
					<a class="red activedel" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;"> <i class="icon-trash bigger-130"></i>
					</a>
...
jQuery(function($){
	$('.activedel').click(function(){
					...
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('activedel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
