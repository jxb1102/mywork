...
<html>
	...
	<div class="row">
		<div class="col-sm-6">
			...
			<form <?php if($info): ?>action="<?php echo U('wuliuedit');?>"
				<?php else: ?>
				action="<?php echo U('wuliuadd');?>"<?php endif; ?>
			...
			<div class="form-group">
				...
				<div class="col-xs-12 col-sm-9">
					<div class="clearfix">
						<input type="file" name="filepic" id="pic" class="col-xs-12 col-sm-6" value="<?php echo ($info["uri"]); ?>" />
					</div>
				</div>
			</div>
		<div class="form-group">
...
<div class="form-group">
				...
				<div class="col-xs-12 col-sm-9">
					<div class="clearfix">
						<input type="text" name="sort" id="sort" class="col-xs-12 col-sm-6" value="<?php echo ($info["sort"]); ?>" placeholder="数字越大越靠前显示" />
					</div>
				</div>
			</div>
<div class="form-group">
	...
	<div class="col-xs-12 col-sm-9">
		<div class="clearfix">
			<?php if($info): ?><input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
				...
				<button type="submit" class="btn btn-success btn-next no-border">确认编辑</button><a href="<?php echo U('wuliu');?>" class="btn btn-light btn-next no-border">取消</a>
				<?php else: ?>
...
<tbody>
	<?php if(is_array($list)): $i = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			...
			<td class="action-buttons">
				<a href="<?php echo ($v["pic"]); ?>" target="_blank">图片</a>
				<a class="blue" href="<?php echo U('wuliu','id='.$v['id']);?>">
					<i class="icon-pencil bigger-130"></i>
				</a>

				<a class="red wuliudel" href="###" sid="<?php echo ($v["id"]); ?>" onclick="return false;">
...
jQuery(function($){
	$('.wuliudel').click(function(){
					var id=$(this).attr('sid');
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('wuliudel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
