...
<tbody>
	<?php if(is_array($catelist)): $i = 0; $__LIST__ = $catelist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
			<td class="center"><?php echo ($v["id"]); ?></td>

			<td><a href="<?php echo U('Post/goodlist','fid='.$v['id']);?>"><?php echo ($v["title"]); ?></a></td>
			<td><?php echo ($v["sort"]); ?></td>
			<td><?php echo (getstatus($v["state"])); ?></td>
			<td class="action-buttons">
				<a class="blue" href="<?php echo U('goodcate','id='.$v['id']);?>">
					<i class="icon-pencil bigger-130"></i>
				</a>
				
				<a class="red delgoodcate" href="javascript:;" sid="<?php echo ($v["id"]); ?>" onclick="return false;">
		...
		<div class="col-sm-8">
			...
			<form <?php if($info): ?>action="<?php echo U('goodcateedit');?>"
				<?php else: ?>
				action="<?php echo U('goodcateadd');?>"<?php endif; ?>
			...
			<div class="form-group">
				...
				<div class="col-xs-12 col-sm-10">
					<div class="clearfix">
						<label class="blue">
							<input type="radio" name="state"  class="ace" value="1" <?php if(($info["state"]) == "1"): ?>checked<?php endif; ?> <?php if($info == ''): ?>checked<?php endif; ?> />
							<span class="lbl">启用</span>
						</label>
						 <label class="blue">
							<input type="radio" name="state"  class="ace" value="0" <?php if(($info["state"]) == "0"): ?>checked<?php endif; ?>/>
	...
	<div class="col-xs-12 col-sm-10">
		<div class="clearfix">
			<?php if($info): ?><input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
				<button type="submit" class="btn btn-success btn-next no-border">确认编辑</button><a href="<?php echo U('goodcate');?>" class="btn btn-light btn-next no-border">取消</a>
				<?php else: ?>
...
jQuery(function($){
	$('.delgoodcate').click(function(){
					var id=$(this).attr('sid');
					bootbox.confirm("确定要删除么?", function(result){
						if(result) {
							$.post("<?php echo U('goodcatedel');?>",{'id':id,'ajax':1},function(data){
								if (data.status) {
									$.gritter.add({
