...
<html>
	...
	<div class="row">
		<div class="col-sm-12">
			<form <?php if($info): ?>action="<?php echo U('dianlistedit');?>"
				<?php else: ?>
				action="<?php echo U('dianlistsave');?>"<?php endif; ?>
			class="form-horizontal" id="validation-form" method="post" enctype="multipart/form-data">
			<div class="form-group">
				<label class="control-label col-xs-12 col-sm-2 no-padding-right" for="title">店铺名称:</label>
			...
			<div class="form-group">
				...
				<div class="col-xs-12 col-sm-9">
					<div class="clearfix">
						<input type="text" name="url" id="url" class="col-xs-12 col-sm-6" value="<?php echo ($info["url"]); ?>" placeholder="点击图片显示的网址" />
					</div>
				</div>
			</div>
			<div class="form-group">
				...
				<div class="col-xs-12 col-sm-10">
					<div class="clearfix">
						<select name="cid" class="input-medium" id="cid">
							...
							<?php if(is_array($dianlei)): $i = 0; $__LIST__ = $dianlei;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$c): $mod = ($i % 2 );++$i;?><option value="<?php echo ($c["id"]); ?>" <?php if(($c["id"]) == $info["fid"]): ?>selected<?php endif; ?>><?php echo ($c["title"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
						</select>
					</div>
				</div>
			</div>
			...
			<div class="form-group">
				<label class="control-label col-xs-12 col-sm-2 no-padding-right" for="pic">网店Logo:</label>

				<div class="col-xs-12 col-sm-9">
					<div class="clearfix">
						<input type="file" name="filepic" id="pic" class="col-xs-12 col-sm-6" />(204*204)
					</div>
				</div>
			</div>
		<div class="form-group">
				
			<label class="control-label col-xs-12 col-sm-2 no-padding-right" for="jianjie">网店简介:</label>

			<div class="col-xs-12 col-sm-10">
				<div class="clearfix">
					<textarea class="col-sm-10" name="jianjie" style="height: 100px;" id="jianjie" placeholder="请简单描述，不要超过60个字"><?php echo ($info["jianjie"]); ?></textarea>
				</div>
			</div>
		</div>
<div class="form-group">
	...
	<div class="col-xs-12 col-sm-10">
		<div class="clearfix">
			<?php if($info): ?><input type="hidden" name="id" value="<?php echo ($info["id"]); ?>">
				...
				<button type="submit" class="btn btn-success btn-next no-border">确认编辑</button><a href="<?php echo U('dianlist');?>" class="btn btn-light btn-next no-border">取消</a>
				<?php else: ?>
				<button type="submit" class="btn btn-success btn-next no-border">确认添加</button><a href="<?php echo U('dianlist');?>" class="btn btn-light btn-next no-border">取消</a><?php endif; ?>
		</div>
	</div>
</div>
